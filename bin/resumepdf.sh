...
wkhtmltopdf --no-background --margin-bottom 0mm --margin-top 0mm --margin-left 0mm --margin-right 0mm ${URL}?pdf=1 ${NORMAL_PDF}
${SUDO_DO} chmod 0770 ${NORMAL_PDF}
${SUDO_DO} chown $DEV_USR:$APACHE_USR_GRP ${NORMAL_PDF}

PRINT_PDF="${DEST}/cv_print.pdf"
wkhtmltopdf --no-background --margin-bottom 5mm --margin-top 5mm --margin-left 5mm --margin-right 5mm ${URL}?printpdf=1 ${PRINT_PDF}
